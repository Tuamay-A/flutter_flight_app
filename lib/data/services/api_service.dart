...
class ApiService extends GetxService {
  ...
  Future<String?> getGuestToken() async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final guestToken = GuestTokenResponse.fromJson(
          response.data,
        ).guestToken;
  ...
  Future<FlightShoppingResponse?> searchFlights(
    ...
    try {
      ...
      print(
        'Flight search API returned non-200. Using offline fallback results.',
      );
      return _buildOfflineFallbackResponse(request);
    } catch (e) {
      print('Error searching flights: $e');
      print('Using offline fallback results due to API error.');
      return _buildOfflineFallbackResponse(request);
    }
  }

  FlightShoppingResponse _buildOfflineFallbackResponse(
    FlightShoppingRequest request,
  ) {
    final originDestinations = request.originDestinations;
    if (originDestinations.isEmpty) {
      return FlightShoppingResponse(offers: []);
    }

    final segmentList = <Map<String, dynamic>>[];
    for (var i = 0; i < originDestinations.length; i++) {
      final leg = originDestinations[i];
      segmentList.add({
        'airlineCode': 'OD',
        'airlineName': 'Offline Demo Air',
        'flightNumber': '10${i + 1}',
        'departureAirport': leg.departure.airportCode,
        'arrivalAirport': leg.arrival.airportCode,
        'departureDateTime': '${leg.departure.date}T08:00:00',
        'arrivalDateTime': '${leg.departure.date}T10:30:00',
      });
    }
    ...
    final fallbackJson = {
      'offers': [
        {
          'offerId': 'offline-demo-1',
          'pricing': {'total': '199.00', 'currency': 'USD'},
          'flights': [
            {'duration': 'PT2H30M', 'segments': segmentList},
          ],
        },
        {
          'offerId': 'offline-demo-2',
          'pricing': {'total': '249.00', 'currency': 'USD'},
          'flights': [
            {'duration': 'PT3H15M', 'segments': segmentList},
          ],
        },
      ],
    };

    return FlightShoppingResponse.fromJson(fallbackJson);
  }
  ...
  Future<List<PaymentOption>> getPaymentOptions(String bookingLocator) async {
    try {
      final response = await _dio.post(
        ...
        data: {'bookingLocator': bookingLocator},
      );
      if (response.statusCode == 200) {
        return (response.data as List)
            .map((i) => PaymentOption.fromJson(i))
            .toList();
      }
    } catch (e) {
